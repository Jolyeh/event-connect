<script>
  ...
  async function loadData() {
    try {
      ...
      const [evJson, catJson] = await Promise.all([evRes.json(), catRes.json()]);
  ...
  $: filtered = (() => {
    ...
    return base.filter((e) => e.category?.name === activeCategory || e.category === activeCategory);
  })();
</script>
    ...
    <div use:reveal class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-12">
      <div>
        ...
        <h2 class="font-bold text-base-content leading-tight" style="font-size: clamp(1.8rem, 3vw, 2.6rem)">
          Événements <em class="font-light text-primary not-italic">en vedette</em>
        </h2>
      </div>
      <a href="/event/vedette" class="btn btn-outline btn-primary rounded-full gap-2 text-xs uppercase tracking-widest shrink-0 self-start lg:self-auto">
