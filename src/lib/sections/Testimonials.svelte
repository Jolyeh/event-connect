    ...
    {#if average && !loading}
      <div class="flex items-center justify-center gap-2 mb-10 -mt-4">
        <div class="flex gap-0.5">
          {#each Array(5) as _, i}
            <Star size={14} class="{i < Math.round(average) ? 'text-warning fill-warning' : 'text-base-content/15'}" />
          {/each}
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        {#each reviews as r, i}
          <div use:reveal={{ delay: i * 200 }}
            ...
            <!-- Quote -->
            <Quote size={28} class="text-primary/20 mb-4 shrink-0" fill="currentColor" />

            <!-- Étoiles -->
            <div class="flex gap-1 mb-4">
              {#each Array(5) as _, j}
                <Star size={13} class="{j < r.rating ? 'text-warning fill-warning' : 'text-base-content/10'}" />
              {/each}
            </div>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-primary/8">
              <div class="avatar">
                {#if imgSrc(r.user?.image)}
                  <div class="w-10 rounded-full ring-2 ring-primary/20 ring-offset-1 ring-offset-base-200">
                    <img src={imgSrc(r.user.image)} alt="{r.user.firstName}"
                      width={40} height={40} class="rounded-full object-cover" />
                  </div>
                {:else}
                  <div class="w-10 h-10 rounded-full bg-primary/10 ring-2 ring-primary/20
