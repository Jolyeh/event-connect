...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start">
  ...
  <div class="space-y-6">
    <div>
      ...
      <div class="card bg-base-200 border border-base-300 p-4 sm:p-5 space-y-4">
        ...
        >
          {#if loadingPwd}<span class="loading loading-spinner loading-xs mr-1"
            ></span>{/if}
          Mettre à jour
        </button>
      </div>
  ...
  <div>
    ...
    <div>
        ...
        <div class="card bg-base-200 border border-base-300 p-5 space-y-3">
          ...
          <div class="flex items-center gap-1">
            {#each Array(5) as _, i}
              <Star
                size={18}
                class={i < existingReview.rating
                  ? "text-warning fill-warning"
                  : "text-base-content/10"}
              />
            {/each}
            <span class="text-xs text-base-content/30 font-semibold ml-1"
              >{existingReview.rating}/5</span
            >
          </div>
          <!-- Message -->
      ...
      {:else}
        <div
          class="card bg-base-200 border border-base-300 p-4 sm:p-5 space-y-4"
        >
          <div>
            <p
              class="text-[10px] uppercase tracking-widest text-base-content/30 font-semibold mb-2"
            >
              Note
            </p>
            <div class="flex items-center gap-1.5">
              ...
              {/each}
              {#if rating}
                <span class="text-xs text-base-content/25 ml-1 font-semibold"
                  >{rating}/5</span
                >
              {/if}
            </div>
    ...
    <div class="mt-5">
      <div class="flex items-center gap-2 mb-3">
        <div
          class="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center shrink-0"
        >
          <CreditCardIcon size={13} class="text-primary" />
        </div>
        <p
          class="text-[10px] uppercase tracking-[0.25em] text-base-content/35 font-bold"
        >
      ...
      <div class="card bg-base-200 border border-base-300 p-5 space-y-3">
        <!-- Message -->
         <ul class="text-sm text-base-content/70 leading-relaxed">
          <li><span class="text-primary mr-1">•</span>Commission de <span class="text-primary">5% </span> sur chaque billet vendu</li>
          <li><span class="text-primary mr-1">•</span>Pas de frais fixe ou d'abonnement</li>
          <li><span class="text-primary mr-1">•</span>Les frais de transfert sont à votre charge</li>
          <li><span class="text-primary mr-1">•</span>Nous payons les frais de paiement des tickets(les participants paient que le prix du tickets)</li>
         </ul>
      </div>
    </div>
  </div>
